...
module.exports = (req, res, next) => {
    try {
        const token = req.headers.authorization.split(' ')[1]; // extrait du token du header 'Authorization' de la requete entrante
        const decodedToken = jwt.verify(token, process.env.CLE_SECRETE); // fonction verify pour décoder le token 
